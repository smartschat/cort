...
class Visualizer:
    def __init__(self, structured_coreference_analysis, corpus_name, highlight_error=None):
        self.html_header = \
            "<!doctype html>\n" \
            "<html>\n" \
            "\t<head>\n" \
    ...
    def run(self):
        ...
        for document in sorted(self.structured_coreference_analysis.reference.documents,
            ...
            document_mentions = sorted(mentions + system_mentions)

            text_source = self.__generate_html(document, document_mentions)
    ...
    def __generate_html(self, document, mentions):
        document_html = "\n\t\t\t<ol class=\"text\">\n" \
                        "\t\t\t\t<li class=\"sentence\">"
